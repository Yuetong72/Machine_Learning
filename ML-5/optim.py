...
def rmsprop(theta, dtheta, config=None):
  ...
  #############################################################################
  r = config['decay_rate']*config['cache'] + (1-config['decay_rate'])*dtheta*dtheta
  update = -config['learning_rate']/(config['epsilon']+np.sqrt(r)) * dtheta
  next_theta = theta + update
  config.update({"cache": r})
...
def adam(theta, dtheta, config=None):
  ...
  m = b1*m + (1-b1)*dtheta
  v = b2*v + (1-b2)*dtheta*dtheta
  t += 1
  mb = m/(1-b1**t)
  vb = v/(1-b2**t)
  next_theta = theta - a*mb/(np.sqrt(vb) + e)
