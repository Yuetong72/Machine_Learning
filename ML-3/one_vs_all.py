...
class one_vs_allLogisticRegressor:
    ...
    def train(self,X,y,reg):
       ...
       """
        ...
        for k in range(len(self.labels)):
            ytmp = (y==k)*1.0
            clf = linear_model.LogisticRegression(penalty='l2', C=1.0/reg, solver='sag', max_iter=1000, fit_intercept=False)
            clf.fit(X,ytmp)
    ...
    def predict(self,X):
        ...
        ###########################################################################
        ytmp = utils.sigmoid(X@self.theta)
        y_pred = np.argmax(ytmp,axis=1)
