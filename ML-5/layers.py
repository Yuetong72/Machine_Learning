...
def affine_backward(dout, cache):
  ...
  xx = x.reshape((np.shape(x)[0],-1))
  dxx = dout@theta.T
  dx = dxx.reshape(np.shape(x))
  dtheta = xx.T@dout
  dtheta0 = dout.T@np.ones(np.shape(x)[0])
...
def conv_forward_naive(x, theta, theta0, conv_param):
  ...
  xpad = np.pad(x,npad,'constant')

  # initialize output
  if ((H + 2*p - HH) % s != 0):
    raise ValueError('stride size is not compatible')
  ...
  out = np.zeros((m,F,Hout,Wout))

  # Fill in the output data

  # from http://cs231n.github.io/convolutional-networks/ we know
  # (0,0) position of the output using the second data point, third filter, is:
  # out[1,2,0,0] = np.sum(xpad[1,:,(s*0):HH+(s*0),(s*0):WW+(s*0)]*theta[2,:,:,:])+theta0[2]

  for indm in range(m):
    for indF in range(F):
      for indH in range(Hout):
        for indW in range(Wout):
          out[indm,indF,indH,indW] = np.sum(xpad[indm,:,(s*indH):HH+(s*indH),(s*indW):WW+(s*indW)]*theta[indF,:,:,:])+theta0[indF]
...
def conv_backward_naive(dout, cache):
  ...
  # I got stuck at first so I looked at
  # https://github.com/huyouare/CS231n/blob/master/assignment2/cs231n/layers.py
  # It's clear to me now

  # we first retrieve parameters and cache from forward prop
  ...
  for n in range(m): # do the rest of the calculation under each of the m images
    dx_pad = np.pad(dx[n,:,:,:], ((0,),(p,),(p,)), 'constant')
    x_pad = np.pad(x[n,:,:,:], ((0,),(p,),(p,)), 'constant')
    for f in range(F):
      for h_ in range(H_):
        for w_ in range(W_):
          h1 = h_*s # the h beginning of each xpad
          h2 = h_*s + HH
          w1 = w_*s # the w beginning of each xpad
          w2 = w_*s + WW
          dx_pad[:, h1:h2, w1:w2] += theta[f,:,:,:] * dout[n,f,h_,w_]
          dtheta[f,:,:,:] += x_pad[:, h1:h2, w1:w2] * dout[n,f,h_,w_]
          dtheta0[f] += dout[n,f,h_,w_]
    dx[n,:,:,:] = dx_pad[:,p:-p,p:-p]
...
def max_pool_forward_naive(x, pool_param):
  ...
  for i in range(m):
     for c in range(C):
        for k in range(H2):
          for l in range(W2):
            out[i,c,k,l] = np.max(x[i,c,k*s:(k*s+ph), l*s:(l*s+pw)])
...
def max_pool_backward_naive(dout, cache):
  ...
  for i in range(m):
    for c in range(C):
      for k in range(H2):
        for l in range(W2):
          pad_select = x[i,c,k*s:(k*s+ph), l*s:(l*s+pw)]
          pad_max = np.max(pad_select)
          pad_mask = 1*(pad_select==pad_max)
          dpad_select = dout[i,c,k,l]*pad_mask
          dx[i,c,k*s:(k*s+ph), l*s:(l*s+pw)] += dpad_select
